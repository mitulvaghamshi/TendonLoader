...
import 'package:tendon_loader/pages/bluetooth/connected_list.dart';
...
class const HomeScreen({super.key}) extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    children: [
      ...
      const SizedBox(height: 8),
      ButtonFactory.tile(
        onTap: () => const PrescriptionRoute().push<void>(context),
        color: Theme.of(context).primaryColorDark,
      ...
      const SizedBox(height: 8),
      ButtonFactory.tile(
        onTap: () => const SettingScreenRoute().push<void>(context),
...
extension on HomeScreen {
  ...
  Future<void> _safePush(
    ...
    await const SettingScreenRoute().push<void>(context);
  }
  ...
  Future<void> _connectProgressor(BuildContext context) async {
    ...
    await Progressor.instance.stopListening();
    GraphHandler.reset();
  }
