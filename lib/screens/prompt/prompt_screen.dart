...
Future<void> main() async {
  ...
  runApp(MaterialApp(
    ...
    home: AppScope(
      data: state,
      child: const PromptScreen(exportKey: ''),
    ),
  ));
}

class PromptScreen extends StatefulWidget {
  const PromptScreen({super.key, required this.exportKey});
...
class PromptScreenState extends State<PromptScreen> {
  late final bool _autoUpload = AppState.of(context).isAutoUpload();
  double? painScore;
...
extension on PromptScreenState {
  ...
  Future<void> _onFinished() async {
    final Export export =
        await AppState.of(context).getExportFor(widget.exportKey);
